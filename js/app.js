(() => {
    ...
    const socialButton = document.querySelector(".top-header__social");
    const phoneBlock = document.querySelector(".block");
    const socialBlock = document.querySelector(".top-header__button");
    const search = document.querySelector(".icon-magnifying-glass>input");
    const searchButton = document.querySelector(".icon-magnifying-glass");
    adaptHeader();
    window.addEventListener("resize", adaptHeader);
    function adaptHeader() {
        if (document.documentElement.clientWidth <= 769) {
            phoneBlock.appendChild(socialButton);
            searchButton.addEventListener("click", (() => search.classList.toggle("pc")));
        } else if (document.documentElement.clientWidth > 769) socialBlock.before(socialButton);
    }
    const menuBurgerIc = document.querySelector(".menu-burger__icon");
    const menuBurgerSublist = document.querySelectorAll("#sublist");
    const menuBurgerSuplist = document.querySelectorAll("#suplist");
    const listWrapper = document.querySelector(".burger-menu__list-box");
    const menuBurger = document.querySelector(".menu-burger");
    menuBurgerIc.addEventListener("click", (() => {
        document.body.classList.toggle("menu-open");
    }));
    menuBurger.addEventListener("click", openMenuBurger);
    function openMenuBurger(event) {
        const menuBurgerClose = event.target.closest(".burger-menu__close");
        const menuBurgerBack = event.target.closest(".burger-menu__back");
        const menuBurgerNext = event.target.closest(".icon-cheveron-up");
        if (menuBurgerNext) {
            menuBurgerNext.parentNode.classList.add("hidden");
            menuBurgerNext.parentNode.classList.remove("active");
            let link = menuBurgerNext.id;
            if ("list" == link) {
                for (let i of menuBurgerSuplist) if (i.dataset.suplist == menuBurgerNext.dataset.list) {
                    i.classList.add("active");
                    i.classList.add("pc");
                    i.classList.remove("hidden");
                    document.body.classList.add("suplist");
                }
                document.querySelector(".burger-menu__back").style.display = "block";
            } else for (let i of menuBurgerSublist) {
                i.classList.remove("pc");
                if (i.dataset.sublist == menuBurgerNext.dataset.suplink) {
                    i.classList.add("active");
                    i.classList.remove("hidden");
                    i.classList.add("pc");
                    document.body.classList.remove("suplist");
                    document.body.classList.add("sublist");
                }
            }
        }
        if (menuBurgerClose) {
            document.querySelector(".burger-menu__back").style.display = "none";
            for (let i of menuBurgerSublist) {
                i.classList.add("hidden");
                i.classList.remove("active");
                i.classList.remove("pc");
            }
            for (let i of menuBurgerSuplist) {
                i.classList.add("hidden");
                i.classList.remove("active");
                i.classList.remove("pc");
            }
            document.body.classList.remove("menu-open");
            setTimeout((() => listWrapper.classList.remove("hidden")), 800);
        }
        if (menuBurgerBack) {
            if (document.body.classList.contains("sublist")) {
                for (let list of menuBurgerSublist) {
                    list.classList.remove("active");
                    list.classList.add("hidden");
                }
                for (let list of menuBurgerSuplist) {
                    list.classList.add("active");
                    list.classList.remove("hidden");
                }
                document.body.classList.remove("sublist");
                document.body.classList.add("suplist");
            }
            if (document.body.classList.contains("suplist")) {
                for (let list of menuBurgerSuplist) {
                    list.classList.remove("active");
                    list.classList.add("hidden");
                }
    ...
    createProducts();
    const sliderBox = document.querySelector(".swiper-wrapper");
    async function createProducts() {
        let response = await fetch("https://fakestoreapi.com/products");
        let products = await response.json();
        products.forEach((product => {
            const card = `<div class="swiper-slide">\n    <a href="#">\n    <div class="card__photo">\n        <img src="${product.image}" alt="товар">\n    </div>\n    <div class="card__content">\n        <h3>${product.title}</h3> <span> Ціна: ${product.price}$</span>\n    </div>\n</a>\n</div>`;
            sliderBox.insertAdjacentHTML("beforeend", card);
        }));
        setTimeout((() => {
            Swiper.init();
        }), 0);
